...
public class BeanNestedAcess extends BeanSingleAcess {
    ...
    public Class getType(Object bean, String propertyName) throws Exception {
        ...
        for (int i=0; i<properties.length; i++) {
            String property = properties[i];
            if (i == (properties.length - 1)) {
                return super.getType(bean, property);
            } 
            else {
                bean = super.get(bean, property);
            }
    ...
    @Override
    public Class getCollectionType(Object bean, String propertyName) throws Exception {
        String[] properties = propertyName.split("\\.");
        for (int i=0; i<properties.length; i++) {
            String property = properties[i];
            if (i == (properties.length - 1)) {
                return super.getCollectionType(bean, property);
            } 
            else {
                bean = super.get(bean, property);
            }
